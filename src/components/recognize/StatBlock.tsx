...
interface StatBlockProps {
  ...
  value: string;
  icon: string;
  tone: 'yellow' | 'coral' | 'sage';
}

export default function StatBlock({ label, value, icon, tone }: StatBlockProps) {
  const bg = { yellow: HP_TOKENS.yellowSoft, coral: HP_TOKENS.coralSoft, sage: HP_TOKENS.sageSoft }[tone];
  const fg = { yellow: '#8A6814', coral: '#B5574A', sage: HP_TOKENS.sage }[tone];
